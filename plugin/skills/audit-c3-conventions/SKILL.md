...
6. **Evaluates each expectation** against the current working directory. For an MCP server, the version checked against `minVersion` is **the plugin's own pin** in `${CLAUDE_PLUGIN_ROOT}/.claude-plugin/plugin.json` `mcpServers` — the server the agents actually talk to — and reachability is probed by running that exact pinned spec, `npx -y <package>@<pin> --version` (e.g. `@genvidtech/construct3-chef@2.0.0`), from a fresh temporary directory, so the result does not depend on where the audit was run or what the repo has installed.
...
- **MCP server not reachable** — the audit probes each server by running its pinned spec, `npx -y <package>@<pin> --version` (the scoped `@genvidtech/construct3-chef` / `@genvidtech/c3-domain-manager` at the version the plugin's `plugin.json` `mcpServers` launches), from a fresh temporary directory. A failure means npx could not fetch or run that exact version — check network/registry access and that the pinned version is published. A copy of the package installed in your repo neither helps nor hurts this check.
- **MCP server version too old** — the plugin's own pin in its `plugin.json` `mcpServers` is below a component's `minVersion`, so the plugin is internally inconsistent; this fails the same way in every repo. Update the plugin (or, as its maintainer, bump the pin). Your repo's installed packages play no part in this check.
- **No pinned version found** — the plugin's `plugin.json` has no `<package>@<x.y.z>` argument for that server. This is a defect in the plugin itself, not in your repo; update or reinstall the plugin.
