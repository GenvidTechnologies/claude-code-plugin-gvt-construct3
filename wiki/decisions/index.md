...
* [0022. The Audit Adopts @genvidtech/audit-core Behind a Usability Preflight, and commands.validate Installs plugin/ Dependencies](0022-audit-core-adoption-and-npm-ci-in-validate.md) - `audit-c3-conventions` takes its mechanism from `@genvidtech/audit-core`, pinned exactly at 0.2.0, and keeps its policy in local resolver wrappers. A thin CLI checks that the package can be imported and exits 2 with a remediation message when it cannot. `commands.validate` now runs `npm ci` for `plugin/`, retiring ADR 0019's CI-only asymmetry, and the ADR-0014 mirrored tests change only their import line.
