...
* [Skill authoring conventions](skill-authoring-conventions.md) - Frontmatter keys are fixed; scripts split a pure lib from a thin I/O CLI with tests at a path validation actually globs; remediation prose must never describe a check the script does not yet implement; and prose must never restate a value a declarative source already owns.
