...
> This repo's conventions, consumed by `/gvt-dev:maintain-wiki`. It is the
> **maintenance schema** for the three-tier wiki: `raw/` (immutable captured
> sources) → `wiki/` (LLM-maintained pages, `index.md`, `log.md`) → this
> schema (the rules that govern how the first two are kept in sync).
>
> The section headings are load-bearing — the skill locates guidance by
> heading. Edit the prose under a heading, never the heading itself.

**Two OKF bundles live in this repo, and they are not the same tier.** `wiki/`
is the **dev-workspace** wiki described by this schema — how maintaining *the
plugin* works. `plugin/docs/c3/` is the **shipped** C3 platform reference,
which is also an OKF v0.2 bundle so a consuming repo's wiki tooling can read
it, but it is **not** maintained by `ingest`: it ships to consumers, it is
version-controlled against `construct3-sample` per
[ADR 0008](decisions/0008-recording-verification-provenance-in-docs-c3.md), and
it has no `raw/` tier of its own. Never fold one into the other — that is the
[three knowledge boundaries](decisions/0001-three-knowledge-boundaries.md) rule
applied to the wiki tier.

Pages under `wiki/` follow the [Open Knowledge Format (OKF) v0.2](https://github.com/GoogleCloudPlatform/knowledge-catalog/blob/main/okf/SPEC.md), pinned at upstream commit `3fcbb9f828c2f23d109c855ee403c3a4c81f3a96`. A later spec revision obliges different things depending on its scope:
...
- **`sources[]`** — §5.1. `resource` is **required** within each entry. `id`
  ...
  one citing the immutable capture at `../raw/<capture>.md`, one citing
...
**This repo's rule — key `stale_after` to what invalidates the page, and in
this repo the dominant invalidator is a pinned MCP server bump.**

- **Anything whose truth is pinned to a `construct3-chef` / `c3-domain-manager`
  version** — tool-surface counts, agent allow-list sizes, the `resolveRootFolder`
  mirror, `expects` version floors — gets `stale_after` **6 months** out. Bumps
  land every few weeks and, per this repo's own history, the bump issue's
  surface table is wrong more often than not, so these pages decay fast.
- **Practice and procedure pages** (how to ground a claim in chef's source, how
  triage labels work, how a pin bump is verified) get **1 year**.
- **Pages restating a settled boundary or split** (the three knowledge
  boundaries, the `plugin/` vs. repo-root split) **omit `stale_after`** — they
  change only by a superseding ADR, and an ADR is the signal, not a date.

A page that says "as of chef X.Y.Z" in its body always carries a
`stale_after`, whatever its topic.
...
- **`lint`** — an advisory health check, not a mutation. It flags: dead
  ...
  links (a link that resolves outside `wiki/` — legal, but unresolvable
  to a consumer that receives only the bundle), orphaned pages (a page listed
  in **no** index — neither `wiki/index.md` nor its own subdirectory's
...
`sources[].resource` in a page's frontmatter (`../raw/<capture>.md`,
§6.2's own worked-example relative-path form) — wiki-links relate living
`wiki/` pages to each other; `sources` records provenance back to the
immutable tier.
